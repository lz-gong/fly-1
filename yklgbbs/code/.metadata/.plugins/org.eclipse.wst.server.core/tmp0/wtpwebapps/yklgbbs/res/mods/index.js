        ...
        picture: function(editor){ //插入图片
          layer.open({
            ...
            ,success: function(layero, index){
              ...
              layui.upload({
                /*url: '/api/upload/'*/
            	  url: '/yklgbbs/imgUploadServlet.do'
                ,elem: '#fly-jie-upload .layui-upload-file'
                ,success: function(res){ //这里的res对应一个类对象，所以在bean中新建了一个Image类
...
//                	  alert('图片最大不能超过500kb！');
                	  ...
                	  window.location.href="/yklgbbs/jsp/login.jsp";
                  }
                }
              });
...
//                layui.focusInsert(editor[0], 'img['+ field.image + '] ');
                layui.focusInsert(editor[0], 'img['+ field.image + ']img');
                layer.close(index);
              });
            }
          });
...
//            layui.focusInsert(editor[0], ' a('+ val +')['+ val + '] ');
            layui.focusInsert(editor[0], 'a['+ val +']a'+val+'[/a]');
            layer.close(index);
          });
        }
...
//            layui.focusInsert(editor[0], '[pre]\n'+ val + '\n[/pre]');
        	  layui.focusInsert(editor[0], '[pre]'+ val + '[/pre]');
            layer.close(index);
          });
        }
...
//            ,content: '<div class="detail-body" style="margin:20px;">'+ content +'</div>'
            //修改后
            ,content: '<div class="detail-body" style="margin:20px;">'+ replaceAll(getFaceConfig(),replaceAll(getContentConfig(),L_content.value)) +'</div>'
          });
        }
      };
